...
/**
 * A six-value matrix global variable in a shader program.
 * @internal
 */
export default abstract class MatrixUniform6 extends MatrixUniform {
	/**
	...
	public constructor(program: Program, activeInfo: WebGLActiveInfo) {
		...
		this.valueCache = [0, 0, 0, 0, 0, 0];
		this.matrixValueCache = [0, 0, 0, 0, 0, 0];
	}
	 ...
	 * @internal
	 */
	protected matrixValueCache: [number, number, number, number, number, number];
	...
	public override iterableSetter(value: Iterable<number>) {
		...
		this.matrixValueCache = [i0, i1, i2, i3, i4, i5];
	}
